...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-bg-elevated border border-border-bright rounded px-3 py-2 text-sm shadow-lg">
      ...
      <div className={`font-numbers font-medium ${isNeg ? 'text-negative' : 'text-positive'}`}>
...
export default function NetIncomeChart({ charts }: Props) {
  const data = useMemo(() =>
    charts.dates.map((date, i) => ({
      date,
      value: charts.net_income[i] != null ? +(charts.net_income[i]! / 1e9).toFixed(2) : null,
    })).reverse(),
  [charts]);

  return (
    <div className="bg-bg-surface border border-border rounded p-4">
      <div className="text-[11px] uppercase tracking-widest text-text-secondary font-medium mb-4">Net Income (B)</div>
